...
<head>
<title>{{ get_option('site_title', 'پیش فاکتور') }}</title>
								...
								<tbody>
									 <tr>
										<td>
											...
											{!! get_company_field($quotation->company_id,'vat_id') != '' ? _lang('ارزش افزوده').': '.clean(get_company_field($quotation->company_id,'vat_id')).'<br>' : '' !!}
											{!! get_company_field($quotation->company_id,'reg_no')!= '' ? _lang('شماره ثبت').': '.clean(get_company_field($quotation->company_id,'reg_no')).'<br>' : '' !!}
										</td>
										<td class="wp-100">
						...
						<td colspan="2" class="pt-5">
							...
							<div class="invoice-col-6 pt-3">
								<div class="d-inline-block float-md-right">
									<h5><b>{{ _lang('جزئیات پیش فاکتور') }}</b></h5>
									<b>{{ _lang('Quotation') }} #:</b> {{ $quotation->quotation_number }}<br>
									<b>{{ _lang('Quotation Date') }}:</b> {{ jdate($date_format, strtotime( $quotation->quotation_date)) }}<br>
								</div>
							</div>
						</td>
					 </tr>
			...
			<table class="table table-bordered mt-2" id="invoice-item-table">
				 <thead class="base_color">
					 <tr>
						 ...
						 <th class="no-print">مالیات</th>
						 <th class="text-right">درصد مالیات</th>
						 <th class="text-right">جمع جزء</th>
					 </tr>
				 </thead>
				 <tbody id="invoice">
