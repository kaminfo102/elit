...
<div class="row">
  <!-- Panel 1 -->
  <div class="col-12">
     <span class="panel-title d-none">پیام رسان</span>
		...
		<div id="frame" class="chat-fullscreen">
			<div id="sidepanel">
				...
				<div id="search">
					...
					<input type="text" id="st" placeholder="جستجوی کاربران" />
				</div>
				@php 
				...
				<div id="contacts" class="tab-content">
								...
								<li class="contact" id="user-{{ $staff->id }}" data-id="{{ $staff->id }}" data-order="{{ array_key_exists($staff->id,$chat_order) ? $chat_order[$staff->id] : $staff->id * 45 }}">
									<div class="wrap">
										...
										<div class="meta">
											...
											<p class="lastemassage">{!! clean(get_last_message($staff->id)) !!}</p>
											<hr>
										</div>
									</div>
								</li>
					...
					<div id="groups" class="tab-pane">
					    <a href="{{ url('live_chat/create_group') }}" data-title="ساخت گروه جدید" class="btn-create-group btn-block ajax-modal">ساخت گروه جدید</a>
				...
				<div id="bottom-bar">
					<button class="nav-link active" data-toggle="tab" data-target="#staff"><i class="fa fa-user"></i> <span>کارمندان</span></button>
					<button class="nav-link" data-toggle="tab" data-target="#groups"><i class="fa fa-users"></i> <span>گروه</span></button>
					@if(Auth::user()->user_type != 'client')
						<button class="nav-link" data-toggle="tab" data-target="#clients"><i class="fa fa-user-circle"></i> <span>کاربران</span></button>
				    @endif
				</div>
			</div>
			<div class="content">
					...
					<div class="group-settings float-right">
					    <div class="dropdown">
						  ...
						  <div class="dropdown-menu">
							<a href="" class="dropdown-item ajax-modal" id="btn-group-members" data-title=" اعضاء گروه">اعضای گروه</a>
							<a href="" class="dropdown-item hidden" id="btn-left-group">ترک گروه</a>
							<a href="" class="dropdown-item hidden ajax-modal" id="btn-edit-group" data-title="ویرایش گروه">ویرایش گروه</a>
							<a href="" class="dropdown-item hidden" id="btn-remove-group">حذف گروه</a>
						  </div>
						</div>
					</div>
				</div>
				<div class="messages">
					<ul>
					    <li class="select-chat-user">
							...
							{{ Auth::user()->name.' '.'عزیز خوش آمدید' }}<br>
							آماده برای شروع مکالمه خود باشید.
						</li>
					</ul>
				</div>
				<div class="message-input">
					<div class="wrap">
						<form id="chat-form" method="post" autocomplete="off">
							...
							<input type="text" name="message" id="message" placeholder="پیام خود را بنویسید ..." />
