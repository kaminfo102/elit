		...
		<div class="card">
			...
			<div class="card-body">
				<ul class="nav nav-tabs">
				  <li class="nav-item">
				    <a class="nav-link active" data-toggle="tab" href="#project_details">جزئیات پروژه</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#task">وظیفه</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#time_sheet">جدول زمانی</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#milestones">رویدادهای مهم</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#invoices">فاکتور ها</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#expense">هزینه ها</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#files">فایل ها</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#notes">نوشته ها</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link" data-toggle="tab" href="#activity_log">گزارش فعالیت</a>
				  </li>
				  <li class="nav-item">
				    <a class="nav-link ajax-modal" href="{{ action('ProjectController@edit', $project->id) }}" data-title="بروزرسانی پروژه">ویـــرایش</a>
				  </li>
				</ul>
				...
				<div class="tab-content mt-4">
					    ...
					    <div class="row">
					    	<div class="col-lg-6">
							    <table class="table table-bordered">
								    <tr><td>نام</td><td><b>{{ $project->name }}</b></td></tr>
									<tr><td>کلاینت</td><td>{{ $project->client->contact_name }}</td></tr>
									<tr>
										<td>پیشرفت</td>
										<td>
											<div class="progress">
									...
									<tr><td>نوع صورتحساب</td><td>{{ ucwords($project->billing_type) }}</td></tr>
									<tr><td>وضعیت</td><td>{!! clean(project_status($project->status)) !!}</td></tr>
									
									@if($project->billing_type == 'fixed')
										<tr><td>نرخ ثابت</td><td>{{ decimalPlace($project->fixed_rate, $currency) }}</td></tr>
									@endif

									@if($project->billing_type == 'hourly')
										<tr><td>نرخ ساعتی</td><td>{{ decimalPlace($project->hourly_rate, $currency) }}</td></tr>
									@endif

									<tr><td>تاریخ شروع</td><td>{{ jdate("$date_format", strtotime($project->start_date)) }}</td></tr>
									<tr><td>تاریخ پایان</td><td>{{ jdate("$date_format", strtotime($project->end_date)) }}</td></tr>
									<tr>
										<td colspan="2">
											<h4>جزئیات پروژه</h4>
											<hr>
							...
							<div class="col-lg-6">
												...
												<div class="seo-fact sbg2">
													<div class="p-4">
														<div class="seofct-icon">
															...
															<span class="float-right">هزینه کل ساعات</span>
														</div>
	                                   		...
	                                   		<div class="card">
												<div class="seo-fact sbg2">
													<div class="p-4">
														<div class="seofct-icon">
														    <i class="ti-user"></i> 
															<span class="float-right">هزینه ثابت</span>
														</div>
								...
								<div class="row">
									<div class="col-md-12">
										 <h5 class="text-center">اعضای پروژه</h5>
										 <hr>
		                                     ...
		                                     <table id="project_members_table" class="table">
												...
												<tbody>
												    ...
												    <tr data-id="row_{{ $project_member->id }}">
														...
														<td><a href="{{ url('projects/delete_project_member/'.$project_member->id) }}" class="ajax-get-remove">حــذف</a></td>
												    </tr>
												    @endforeach
                    ...
                    <div class="tab-pane" id="task">
                    	<a class="btn btn-info btn-xs mb-4 ajax-modal" data-title="وظیفه جدید" href="{{ route('tasks.create') }}?project_id={{ $project->id }}">
							<i class="ti-plus"></i> جـــدید
						</a>
						<div class="table-responsive">
										...
										<tr>
											...
											<td class="text-center">
												<form action="{{ action('TaskController@destroy', $task['id']) }}" class="text-center" method="post">
													...
													<a href="{{ action('TaskController@edit', $task['id']) }}" data-title="بروزرسانی" class="btn btn-warning btn-xs ajax-modal"><i class="ti-pencil"></i></a>
													{{ csrf_field() }}
                    ...
                    <div class="tab-pane" id="time_sheet">
						...
						<a class="btn btn-info btn-xs mb-4 ajax-modal" data-title="کزارش زمان" href="{{ route('timesheets.create') }}?project_id={{ $project->id }}">
								...
								<tbody>
									@foreach($timesheets as $timesheet)
										<tr data-id="row_{{ $timesheet->id }}">
											...
											<td class='start_time'>{{ jdate("$date_format $time_format",strtotime($timesheet->start_time)) }}</td>
											<td class='end_time'>{{ jdate("$date_format $time_format",strtotime($timesheet->end_time)) }}</td>
												...
												<div class="dropdown">
												  ...
												  <form action="{{ action('TimeSheetController@destroy', $timesheet['id']) }}" method="post">
													...
													<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
														<a href="{{ action('TimeSheetController@edit', $timesheet['id']) }}" data-title="بروزرسانی زمان گزارش" class="dropdown-item ajax-modal"><i class="mdi mdi-pencil"></i> ویرایش</a>
														<a href="{{ action('TimeSheetController@show', $timesheet['id']) }}" data-title="{{ $timesheet->task->title }}" class="dropdown-item ajax-modal"><i class="mdi mdi-eye"></i> نمایش</a>
                    ...
                    <div class="tab-pane" id="milestones">
                    	<a class="btn btn-info btn-xs mb-4 ajax-modal" data-title="رویداد مهم" href="{{ route('project_milestones.create') }}?project_id={{ $project->id }}"><i class="ti-plus"></i> رویداد مهم</a>
							...
							<table id="project_milestones_table" class="table">
								...
								<tbody>
									...
									<tr data-id="row_{{ $projectmilestone->id }}">
										...
										<td class='due_date'>{{ jdate("$date_format",strtotime($projectmilestone->due_date)) }}</td>
										...
										<td class="text-center">
											<div class="dropdown">
											  ...
											  <form class="ajax-remove" action="{{ action('ProjectMilestoneController@destroy', $projectmilestone['id']) }}" method="post">
												...
												<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
													<a href="{{ action('ProjectMilestoneController@edit', $projectmilestone['id']) }}" data-title="بروزرسانی رویداد مهم" class="dropdown-item ajax-modal"><i class="mdi mdi-pencil"></i> ویرایش</a>
													<a href="{{ action('ProjectMilestoneController@show', $projectmilestone['id']) }}" data-title="نمایش رویداد" class="dropdown-item ajax-modal"><i class="mdi mdi-eye"></i> نمایش</a>
													<button class="dropdown-item" type="submit"><i class="mdi mdi-delete"></i> حذف</button>
												</div>
											  </form>
											</div>
										</td>
										...
										<td class="text-center">
											<div class="dropdown">
											  ...
											  <form class="ajax-remove" action="" method="post">
												...
												<div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
													<a href="" data-title="" class="dropdown-item ajax-modal"><i class="mdi mdi-pencil"></i> ویرایش</a>
													<a href="" data-title="" class="dropdown-item ajax-modal"><i class="mdi mdi-eye"></i> نمایش</a>
													<button class="dropdown-item" type="submit"><i class="mdi mdi-delete"></i> حذف</button>
												</div>
											  </form>
											</div>
                    ...
                    <div class="tab-pane" id="invoices">
                    	...
                    	<a href="{{ route('invoices.create') }}?related_to=projects&project_id={{ $project->id }}" class="btn btn-info btn-xs mb-4"><i class="ti-plus"></i> جــدید</a>
							...
							<table id="expense-table" class="table table-bordered">
								...
								<tbody>
									@foreach($expenses as $expense)
										<tr>
											<td class='trans_date'>{{ jdate("$date_format",strtotime($expense->trans_date)) }}</td>
											...
											<td class="text-center">
												...
												<div class="dropdown">
													...
													<i class="fa fa-angle-down"></i></button>
													<ul class="dropdown-menu">
														<a class="dropdown-item ajax-modal" data-title="ویرایش هزینه" href="{{ action('ExpenseController@edit', $expense->id) }}"><i class="fas fa-edit"></i> ویرایش</a>
														
														<a class="dropdown-item ajax-modal" href="{{ action('ExpenseController@show', $expense->id) }}" data-title="نمایش هزینه"><i class="fas fa-eye"></i> نمایش</a>
													
														<form action="{{action('ExpenseController@destroy', $expense['id'])}}" method="post">									
															{{ csrf_field() }}
															<input name="_method" type="hidden" value="DELETE">
															<button class="button-link btn-remove" type="submit"><i class="fas fa-trash-alt"></i> حذف</button>
														</form>
							...
							<div class="row">
								...
								<div class="col-md-12">
								    <div class="form-group">
									    <label class="control-label">آپلود فایل</label>
								...
								<div class="col-md-12">
								    <div class="form-group">
									    <button type="submit" class="btn btn-primary">آپــلود</button>
								    </div>
								</div>
							</div>			
					    </form>
					    <table id="files_table" class="table table-bordered">
							...
							<tbody>
							    ...
							    <tr data-id="row_{{ $projectfile->id }}">
									...
									<td class='user_id'><a href="{{action('StaffController@show', $projectfile->user->id)}}" data-title="klhda hxghuhj ;hvlkn"class="ajax-modal-2">{{ $projectfile->user->name }}</a></td>
									<td class='file'><a href="{{ url('projects/download_file/'.$projectfile->file) }}">{{ $projectfile->file }}</a></td>
									<td class="remove text-center"><a class="ajax-get-remove" href="{{ url('projects/delete_file/'.$projectfile->id) }}">pbt</a></td>
							    </tr>
							    @endforeach
							    <tr data-id="files_id">
					...
					<div class="tab-pane" id="notes">
						<form method="post" class="ajax-screen-submit" autocomplete="off" action="{{ route('projects.create_note') }}">
							...
							<div class="row">
								...
								<div class="col-md-12">
								    <div class="form-group">
									    <button type="submit" class="btn btn-primary">یادداشت جدید</button>
								    </div>
								</div>
							</div>			
					    </form>
