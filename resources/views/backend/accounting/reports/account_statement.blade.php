...
<div class="row">
	<div class="col-12">
		<div class="card">
			...
			<span class="d-none panel-title">صورتحساب</span>
			...
			<div class="card-body">
					...
					<form class="validate" method="post" action="{{ url('reports/account_statement/view') }}">
						<div class="row">
						  ...
						  <div class="col-md-2">
							...
							<div class="col-md-3">
								...
								<label class="control-label">نوع تراکنش</label>						
									<select class="form-control select2" name="trans_type" id="trans_type" required>
										...
										<option value="dr">بدهی</option>
										<option value="cr">نسیه</option>									
									</select> 
								</div>
							</div>
							...
							<div class="col-md-2">
								<button type="submit" class="btn btn-primary btn-xs mt-26">نمایش گزارش</button>
							</div>
						</form>
				...
				<div class="report-header">
				   <h4>{{ isset($account) ? _lang('Account Statement Of').' '.$acc->account_title : 'صورتحساب' }}</h4>
				   <h5>{{ isset($date1) ? jdate($date_format, strtotime($date1)).' '.'تا'.' '.jdate($date_format, strtotime($date2)) : '-------------  '.'تا'.'  -------------' }}</h5>
				</div>
				...
				<table class="table table-bordered report-table">
					<thead>
						<th>تاریخ</th>
						<th>توضیحات</th>
						<th class="text-right">بدهی</th>    
						<th class="text-right">نسیه</th>       
					</thead>
					<tbody>
					...
					@foreach($report_data as $report) 
					    ...
					    @endif
						<tr>
						   <td>{{ jdate($date_format, strtotime($report->date)) }}</td>
						   <td>{{ $report->note }}</td>
