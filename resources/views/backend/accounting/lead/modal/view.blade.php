...
<ul class="nav nav-tabs" id="leads-tab">
  <li class="nav-item">
    <a class="nav-link active" data-toggle="tab" href="#lead_details">جزئیات پیگیری</a>
  </li>
  <li class="nav-item">
  ...
  <li class="nav-item">
    <a class="nav-link ajax-modal" href="{{ action('LeadController@edit', $lead->id) }}" data-title="بروزرسانی">ویرایش </a>
  </li>
</ul>
...
<div class="tab-content mt-4">
	<div class="tab-pane active" id="lead_details">
		<table class="table table-bordered">
			<tr>
				...
				<td>
					...
					@if($lead->converted_lead == null)
						<a href="{{ route('leads.convert_to_customer', $lead->id) }}" class="btn btn-primary btn-xs float-right"><i class="fas fa-user-tie"></i> تبدیل به مشتری</a>
					@else
						<button class="btn btn-info btn-xs float-right"><i class="fas fa-user-tie"></i> تبدیل به مشتری</button>
					@endif
				</td>
			</tr>
			...
			<tr><td>تاریخ تماس</td><td>{{ jdate($date_format, strtotime($lead->contact_date)) }}</td></tr>
			...
			<tr><td>ارزش افزوده</td><td>{{ $lead->vat_id }}</td></tr>
			<tr><td>تاریخ ثبت</td><td>{{ $lead->reg_no }}</td></tr>
			<tr><td>شهر</td><td>{{ $lead->city }}</td></tr>
			<tr><td>وضعیت</td><td>{{ $lead->state }}</td></tr>
			<tr><td>کد پستی</td><td>{{ $lead->zip }}</td></tr>
			<tr><td>آدرس</td><td>{{ $lead->address }}</td></tr>
		</table>
	</div>

	<div class="tab-pane" id="quotations">
		<a href="{{ route('quotations.create') }}?related_to=leads&lead_id={{ $lead->id }}" class="btn btn-info btn-xs mb-4"><i class="ti-plus"></i> جــدید</a>
						...
						<td class="text-center">
							<div class="dropdown">
								...
								<div class="dropdown-menu">
									...
									<a class="dropdown-item" href="{{ action('QuotationController@show', $quotation->id) }}"><i class="fas fa-eye"></i> نمایش
	...
	<div class="tab-pane" id="files">
		<form method="post" class="ajax-screen-submit" autocomplete="off" action="{{ route('leads.upload_file') }}" enctype="multipart/form-data">
			...
			<div class="row">
				...
				<div class="col-md-12">
				    <div class="form-group">
					    <button type="submit" class="btn btn-primary">بــارگــزاری</button>
				    </div>
				</div>
			</div>			
	    </form>
	    <table id="files_table" class="table table-bordered">
			...
			<tbody>
			    ...
			    <tr data-id="row_{{ $projectfile->id }}">
					...
					<td class='user_id'><a href="{{action('StaffController@show', $projectfile->user->id)}}" data-title="اطلاعات کارمند"class="ajax-modal-2">{{ $projectfile->user->name }}</a></td>
					<td class='file'><a href="{{ url('leads/download_file/'.$projectfile->file) }}">{{ $projectfile->file }}</a></td>
					<td class="remove text-center"><a class="ajax-get-remove" href="{{ url('leads/delete_file/'.$projectfile->id) }}">حذف</a></td>
			    </tr>
			    @endforeach
			    <tr data-id="files_id">
	...
	<div class="tab-pane" id="notes">
		<form method="post" class="ajax-screen-submit" autocomplete="off" action="{{ route('leads.create_note') }}">
			...
			<div class="row">
				...
				<div class="col-md-12">
				    <div class="form-group">
					    <button type="submit" class="btn btn-primary">یادداشت جــدید</button>
				    </div>
				</div>
			</div>			
	    </form>
