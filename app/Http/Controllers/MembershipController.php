...
class MembershipController extends Controller
{
    ...
    {
    	date_default_timezone_set(get_option('timezone','Asia/tehran'));
    }
	...
	*/
	...
	public function pay(Request $request)
    {
		...
		$payment->method = "";
		$payment->currency = get_option('currency','IRR');
		...
		if($request->gateway == "PayPal"){
			...
			$session = \Stripe\Checkout\Session::create([
			  ...
			  'line_items' => [[
			    'price_data' => [
			      ...
			      'unit_amount' =>   round(convert_currency(get_option('currency','IRR'), get_option('stripe_currency','IRR'), ($data['amount'] * 100))),
			      'currency' 	=>   get_option('stripe_currency','IRR'),
			    ],
			    'quantity' => 1,
			  ]],
			  'mode' => 'payment',
			...
			$orderData = [
			    'receipt'         => $payment->id,
			    'amount'          => round(convert_currency(get_option('currency','IRR'), 'IRR',($data['amount'] * 100))),
			    'currency'        => 'INR',
	...
	public function paypal($action){
		if($action == "return"){
			return redirect('/dashboard')->with('paypal_success', _lang('متشکرم ، شما با موفقیت عضویت خود را تمدید کردید. اگر می بینید عضویت شما منقضی شده است لطفاً صبر کنید تا ایمیل تأیید دریافت کنید'));
		}else if($action == "cancel"){
			return redirect('membership/extend')->with('message', _lang('پرداخت لغو شد !'));
		}
	}
	...
	public function paypal_ipn(Request $request)
	{
		...
		$amount = convert_currency(get_option('paypal_currency','IRR'), get_option('currency','IRR'), $request->mc_gross);
	...
	public function stripe_payment($action, $payment_id){
		...
		if($action == 'cancel'){
			return redirect('/dashboard')->with('error', _lang('پرداخت لغو شد !'));
		}
        ...
        if($session->amount_total != round(convert_currency(get_option('currency','IRR'), get_option('stripe_currency','IRR'), ($payment->amount * 100)))){
			return redirect('/dashboard')->with('error', _lang('عملیات غیرقانونی!'));
		}
        ...
        return redirect('/dashboard')->with('success', _lang('متشکرم ، شما با موفقیت عضویت خود را ارتقاء دادید.'));
	}
	...
	public function razorpay_payment($payment_id){
	    ...
	    try{
	        ...
	        if($charge->amount != round(convert_currency(get_option('currency','IRR'), 'IRR',($payment->amount * 100)))){
				return redirect('/dashboard')->with('error', _lang('عملیات غیر قانونی'));
			}
