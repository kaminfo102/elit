...
class WebsiteController extends Controller
{
    ...
    public function emaiL_subscribed(Request $request){
        ...
        $validator = Validator::make($request->all(), [
          ...
          'email.unique' => 'با عرض پوزش ، شما قبلاً مشترک شده اید'
        ]);
        ...
        if(! $request->ajax()){
           return back()->with('success', 'با تشکر فراوان از عضویت شما');
        }else{
           return response()->json(['result'=>'success', 'action'=>'store', 'message'=>'با تشکر از عضویت شما','data'=>$emaiL_subscriber]);
        }
       ...
       $this->validate($request, [
        ...
        if(get_option('contact_email') != ''){
            try{
                ...
                echo json_encode(array('result'=>true,'message'=>'پیغام شما با موفقیت ارسال شد.'));
            }catch (\Exception $e) {
                echo json_encode(array('result'=>false,'message'=>'خطایی روی داد ، لطفاً دوباره امتحان کنید!'));
            }        
        }
