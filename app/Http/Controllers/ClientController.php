...
class ClientController extends Controller
{
	...
	public function view_invoice($id){
		...
		if(! $invoice){
			return back()->with('error', 'متاسفانه فاکتوری یافت نشد');
		}		
    ...
    { 
        ...
        create_log('projects', $projectfile->related_id, 'آپــلود فایل');

        //Prefix output
        $projectfile->file = '<a href="'. url('projects/download_file/'.$projectfile->file) .'">'.$projectfile->file .'</a>';
        $projectfile->user_id = '<a href="'. action('StaffController@show', $projectfile->user->id) .'" data-title="'. 'نمایش اطلاعات کارکنان' .'"class="ajax-modal-2">'. $projectfile->user->name .'</a>';
        $projectfile->remove = '<a class="ajax-get-remove" href="'. url('projects/delete_file/'.$projectfile->id) .'">'. 'حذف' .'</a>';

        if(! $request->ajax()){
           return back()->with('success', 'فایل با موفقیت آپلود شد');
        }else{
           return response()->json(['result'=>'success','action'=>'store','message'=>'فایل با موفقیت آپلود شد','data'=>$projectfile, 'table' => '#files_table']);
        }
   ...
   /**
    ...
    public function delete_file(Request $request, $id)
    {
        ...
        if(!$projectfile){
            if(! $request->ajax()){
               return back()->with('error','باعرض پوزش، فقط مدیر یا سازنده می تواند فایل را حذف کند!');
            }else{
               return response()->json(['result'=>'error','message'=>'باعرض پوزش، فقط مدیر یا سازنده می تواند فایل را حذف کند!']);
            }
        ...
        create_log('projects', $id, 'فایل حذف شد');
        
        if(! $request->ajax()){
           return back()->with('success','با موفقیت حذف شد');
        }else{
           return response()->json(['result'=>'success','action'=>'delete','message'=>'با موفقیت حذف شد','id'=>$id, 'table' => '#files_table']);
        }
    ...
    {    
        ...
        create_log('projects', $note->related_id, 'یادداشت جدید');
        ...
        if(! $request->ajax()){
           ...
           return response()->json(['result'=>'success','action'=>'store','message'=>'با موفقیت ذخیره شد', 'data'=>$note, 'table' => '#notes_table']);
        }
   ...
   /**
    ...
    public function delete_note(Request $request, $id)
    {
        ...
        $note->delete();
        create_log('projects', $id, 'حذف یادداشت');
        

        if(! $request->ajax()){
           return back()->with('success','با موفقیت حذف شد');
        }else{
           return response()->json(['result'=>'success','action'=>'delete','message'=>'با موفقیت حذف شد','id'=>$id, 'table' => '#notes_table']);
        }
            ...
            $session = \Stripe\Checkout\Session::create([
              ...
              'line_items' => [[
                'price_data' => [
                  'product_data' => [
                      'name' => 'پرداخت فاکتور',
                      'description' =>'فاکتور'.': '.$data['invoice']->invoice_number,
                   ],
                  'unit_amount' =>  round(convert_currency(get_company_field($data['invoice']->company_id, 'currency','IRR'), get_company_field($data['invoice']->company_id, 'stripe_currency','IRR'), ($amount * 100))),
                  'currency'    =>  get_company_field($data['invoice']->company_id, 'stripe_currency','IRR'),
                ],
                'quantity' => 1,
              ]],
              'mode' => 'payment',
	...
	public function stripe_payment($action, $invoice_id){
        ...
        if($action == 'cancel'){
            return redirect('client/view_invoice/'.md5($invoice_id))->with('error', 'پرداختی لغو شد');
        }
		...
		return redirect('client/view_invoice/'.md5($invoice->id))->with('success', 'با تشکر ، پرداختی با موفقیت انجام شد');
	...
	public function paypal($action, $invoice_id){
		if($action == "return"){
			return redirect('client/view_invoice/'.md5($invoice_id))->with('success', 'با تشکر ، پرداختی با موفقیت انجام شد');
    ...
    public function razorpay_payment($invoice_id){
        ...
        try{
            ...
            return redirect('client/view_invoice/'.md5($invoice_id))->with('error', 'پرداخت لغو شد');
        }    
