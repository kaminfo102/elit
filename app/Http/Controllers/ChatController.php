...
class ChatController extends Controller
{
		...
		$this->middleware(function ($request, $next) {
			...
			if( has_membership_system() == 'enabled' ){
                if( ! has_feature( 'live_chat' ) ){
                    return redirect('membership/extend')->with('message','اشتراک فعلی شما این ویژگی را پشتیبانی نمی کند لطفا اشتراک خود را ارتقاء دهید!');
                }
            }    
    ...
    public function index()
    {
		...
		if(Auth::user()->user_type != 'client'){
			...
			if(count($company_ids) == 0){
				return back()->with('error','با عرض پوزش، در حال حاضر هیچ کسب و کاری با اکانت شما وجود ندارد!');
			}
	...
	public function send_group_message(Request $request)
    {
		if ($request->hasFile('file')){
			...
			if($exists == false){
				echo json_encode(['result'=>false,'message'=>array('error'=>'عملیات غیر قانونی')]);
			    die();
			}
	...
	public function store_group(Request $request)
    {
		...
		return response()->json(['result'=>true,'message'=>'گروه جدید ایجاد شد','data'=>$chat_group,'group_members'=>$chat_group->group_members]);
	}
	...
	public function update_group(Request $request, $id)
    {
		...
		return response()->json(['result'=>true,'message'=>'گروه بروزرسانی شد','data'=>$chat_group,'group_members'=>$chat_group->group_members]);
	...
	public function delete_group(Request $request, $id)
    {
		...
		if($group){
		    ...
		    return response()->json(['result'=>true,'message'=>'گروه حذف شد']);
		}
	...
	public function left_group(Request $request, $id)
    {
		...
		$chat_message->message = "<b style='color:#EA2027'>".$user->name.' '.'خارج شدن از گروه'.'</b>';
		$chat_message->company_id = company_id();
		$chat_message->save();
		...
		return response()->json(['result'=>true,'message'=>'شما با موفقیت از گروه خارج شدید','group_id'=>$id]);
