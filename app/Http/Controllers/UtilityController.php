...
class UtilityController extends Controller
{
		...
		if($store == ''){
           return view('backend.administration.general_settings.settings');
			...
			if(! $request->ajax()){
			   ...
			   return response()->json(['result'=>'success','action'=>'update','message'=>_lang('با موفقیت ذخیره شد')]);
			}
			//return redirect('administration/general_settings')->with('success',_lang('با موفقیت ذخیره شد'));
		}
	}
    ...
    {		
	    if($store == ''){
			...
			if(! $request->ajax()){
			   return redirect()->back()->with('success', _lang('با موفقیت ذخیره شد'));
			}else{
			   return response()->json(['result'=>'success','action'=>'update','message'=>_lang('با موفقیت ذخیره شد')]);
			}
		}
	...
	public function backup_database(){
		...
		return redirect()->back()->with('success', _lang('فایل پشتیبان با مفقیت ساخته شد'));
	}
