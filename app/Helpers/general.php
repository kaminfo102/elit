	...
	function get_last_message($sender)
	{
		 ...
		 if ( $message != NULL ) {
			 if($message[0]->from == $sender){
				...
				<p class="float-right time-ago">'. \Carbon\Carbon::parse($message[0]->created_at)->diffForHumans(null,true,true) .' قبل</p>';
				 
				return $string; 
			 }else{
				$string = '<p class="preview">'._lang('شما').': '.$message[0]->message .'</p>
				<p class="float-right time-ago">'. \Carbon\Carbon::parse($message[0]->created_at)->diffForHumans(null,true,true) .' قبل</p>';
...
if ( ! function_exists('g_decimal_place'))
{
 ...
 function g_decimal_place($number, $symbol = '', $format = ''){
	...
	if($base_currency == ''){
		$base_currency = get_option('currency','IRR');
...
if (!function_exists('money_format_2')) {
	function money_format_2($floatcurr, $curr = 'IRR'){
		...
		$currencies['IRR'] = array(0, '.', ',');          //  Jamaican Dollar
		...
		if (array_key_exists($curr,$currencies)){
			...
			return number_format($floatcurr, $currencies['IRR'][0], $currencies['IRR'][1], $currencies['IRR'][2]); 
		}
...
if ( ! function_exists('currency')){
	function currency($currency = 'IRR')
	{ 
		if($currency == ''){
			$currency = get_company_option( 'base_currency', get_option('currency','IRR') );
		}
